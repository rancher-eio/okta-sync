...
use authentik_client::apis::core_api::{CoreGroupsPartialUpdateError, core_groups_partial_update};
use authentik_client::models::{Group, PatchedGroupRequest};
use serde_json::Value;
use uuid::Uuid;

#[derive(Debug, Clone, PartialEq, Eq, clap::Parser, serde::Serialize, serde::Deserialize, bon::Builder)]
#[builder(on(String, into))]
#[command(about = "Update a group (partial update)")]
#[command(after_help = "API Reference @ https://api.goauthentik.io/reference/core-groups-partial-update/")]
#[remain::sorted]
pub struct CoreGroupsPartialUpdate {
...
impl From<CoreGroupsPartialUpdate> for PatchedGroupRequest {
  fn from(value: CoreGroupsPartialUpdate) -> Self {
    let CoreGroupsPartialUpdate {
      group_uuid: _,
      name,
      is_superuser,
      parents,
...
impl crate::authentik::traits::GetWithConfiguration for CoreGroupsPartialUpdate {
  type Error = Error<CoreGroupsPartialUpdateError>;
  type Value = Group;
  ...
  async fn get_with_configuration(self, configuration: &Configuration) -> Result<Self::Value, Self::Error> {
    ...
    let group_request = Some(self.into());
    core_groups_partial_update(configuration, &group_uuid.to_string(), group_request).await
  }
}

crate::authentik::macros::defaults!(const DEFAULT_IS_SUPERUSER: bool = false as "false");

crate::authentik::macros::RunAsync!(CoreGroupsPartialUpdate as Wrapper<GetWithConfiguration>);

crate::authentik::macros::From!(
  Error<CoreGroupsPartialUpdateError>
